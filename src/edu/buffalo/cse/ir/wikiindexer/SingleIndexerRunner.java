...
import src.edu.buffalo.cse.ir.wikiindexer.indexer.IndexWriter;
...
public class SingleIndexerRunner {
	...
	protected void cleanup() throws IndexerException {
		//System.out.println("Single Index Runner, cleanup");
		thr.setComplete();
	}
	...
	private class RunnerThread implements Runnable {
		...
		public void run() 
		{
			...
			while (true) 
			{
				//System.out.println("Enters Single Index Runner while loop!");
				etr = pvtQueue.poll();
				if (etr == null) 
				{
					//System.out.println("Enters Single Index Runner if condition level1!");
					if (isComplete)
					{
						//System.out.println("Enters Single Index Runner if condition level2!");
						try {
